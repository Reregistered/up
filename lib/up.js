...
function UpServer (server, file, opts) {
  ...
  if (opts.nextWorkerIndex) {
    this.nextWorkerIndex = opts.nextWorkerIndex;
  }
...
UpServer.prototype.nextWorkerIndex = function () {
...
UpServer.prototype.nextWorker = function (req) {

  return this.workers[this.nextWorkerIndex(req,this.workers.length)];
};
...
UpServer.prototype.defaultWS = function (req, res, next) {
  ...
  if(this.workers.length && matcher && matcher.length > 2) {
    ...
    next(this.nextWorker(req).port);
  } else {
    var self = this;
    this.once('spawn', function () {
      next(self.nextWorker(req).port);
    });
  }
};
